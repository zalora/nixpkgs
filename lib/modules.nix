    ...
    fold (opt: res:
      ...
      then
        ...
        let
          ...
          # !!! adding _file to modules breaks modularly declaring arguments, so we don't
          # do it for now on our fork for non-attrs
          options' = toList opt.options.options;
          addModuleFile = m:
            if isAttrs m then { _file = opt.file; } // m
            else m;
          coerceOption = file: opt:
            if isAttrs opt then { _file = file; options = opt; }
            else opt;
